...
signature_int = pow(m_int, private_key.d, private_key.n)
signature = long_to_bytes(signature_int) 

# RSA signature generation routine, since the library does not provide it natively
def validate_signature(signature: bytes, original_msg: str) -> bool:
    with open(f"/etc/ssl/iotronic/node_certs/{original_msg}.pub.pem", "rb") as f:
        pub_K_wrk = RSA.import_key(f.read())
    sig_int = bytes_to_long(signature)
    m_check_int = pow(sig_int, pub_K_wrk.e, pub_K_wrk.n)
...
def encrypt_model(model_bytes, aes_key, pub_key):
    # AES encryption
    cipher_aes = AES.new(aes_key.encode('utf-8'), AES.MODE_ECB)
    enc_model = cipher_aes.encrypt(pad(model_bytes, AES.block_size))

    # RSA encryption of AES key
    cipher_rsa = PKCS1_v1_5.new(pub_key)
    enc_key = cipher_rsa.encrypt(aes_key.encode('utf-8'))

    return enc_model, enc_key

def decrypt_model(encr_result):
    # Decrypt AES key with RSA
    cipher_rsa = PKCS1_v1_5.new(private_key)
    aes_key = cipher_rsa.decrypt(encr_result["encr_key"], None).decode('utf-8')

    # Decrypt model with AES
    cipher_aes = AES.new(aes_key.encode('utf-8'), AES.MODE_ECB)
    decrypted = unpad(cipher_aes.decrypt(encr_result["updated_model"]), AES.block_size)
    decrypted_n_samples = int(unpad(cipher_aes.decrypt(encr_result["n_samples"]), AES.block_size).decode('utf-8'))

    return decrypted, decrypted_n_samples
...
class Worker(Plugin.Plugin):
    ...
    def run(self):
        def start_wamp():
                ...
                async def onJoin(session, details):
                    ...
                    async def notify_join(*args, **kwargs):
                        ...
                        if validate_signature(signature, wrk):
                            workers.add(wrk)
                            LOG.info(f"[WAMP] Added new worker: {wrk}")
                            return f"Hello {wrk}, you correctly joined!"
                        else:
                            LOG.warning(f"[WAMP] Invalid signature from worker: {wrk}")
                            return f"Invalid signature from {wrk}, join rejected."
                    ...
                    async def federated_loop(*args, **kwargs):
                        ...
                        if len(workers) >= 1:
                                try:
                                    for wrk in workers:
                                        ...
                                        session.call(uri, {'signature': signature})
                                except Exception as e:
                                    LOG.error(f"[WAMP] Error notifying workers to start training: {e}")
                        ...
                        while not session.stop_train:
                            ...
                            if len(workers) >= 1:
                                for wrk in workers:
                                    with open(f"/etc/ssl/iotronic/node_certs/{wrk}.pub.pem", "rb") as f:
                                        pub_K_wrk = RSA.import_key(f.read())
                                    encr_model, encr_key=encrypt_model(global_bytes, aes_key,pub_K_wrk)
                                    msg={"encr_model": encr_model, "encr_key": encr_key}
                                    uri = f"iotronic.{wrk}.train_round"
                                    calls.append(session.call(uri, msg))
                                ...
                                try:
                                    ...
                                    for result in results:
                                        tmp_model = Net()
                                        decrypted_bytes_model, n_samples = decrypt_model(result)
                                        bytes_to_model(tmp_model, decrypted_bytes_model)
                                    ...
                                    for wrk in list(workers):  # To avoid modification during iteration
                                        try:
                                            ...
                                            await session.call(f"iotronic.{wrk}.stop_training", msg)
        ...
        threading.Thread(target=start_wamp, name="WAMP_Secure_FLMaster", daemon=True).start()
        LOG.info("[WAMP] Master set, waiting for RPC...")
        self.q_result.put("WAMP_Secure_FLMaster plugin correctly started") # Used to notify the correct start of the plugin to S4T
